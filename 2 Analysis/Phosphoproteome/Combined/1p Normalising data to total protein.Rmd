...
# Create a list of the sums of protein intensity for each sample

protein_sums <- apply(total_proteome[,4:ncol(total_proteome)], 2, sum)


# Normalise the phospho data to the total protein intensity in each sample
data_irs <- data_irs %>% dplyr::select(-contains("P")) # Remove the pools from the dataframe

phospho_normalised <- data_irs / protein_sums
...
phospho_normalised <- cbind(phospho_normalised, data_start[,61:ncol(data_start)])

master_list <- cbind(phospho_normalised[49:ncol(phospho_normalised)], phospho_normalised[1:48])

write.csv(master_list, "All_phospho_normalised_total_protein.csv", row.names = F)
...
write.table(WTeJTKcomb, file = "WTeJTKcomb-p2.txt", sep = "\t", row.names = F, col.names = T, quote = F)
write.table(CKOeJTKcomb, file = "CKOeJTKcomb-p2.txt", sep = "\t", row.names = F, col.names = T, quote = F)
