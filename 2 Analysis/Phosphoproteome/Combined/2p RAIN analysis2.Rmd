...
data_start <- read.csv("All_phospho_normalised_total_protein.csv", stringsAsFactors=FALSE)
...
write.csv(WT_results[WT_results$pVal <= 0.05,], "WT_phosphoproteome_all_significant_RAIN2.csv", row.names = F)
write.csv(CKO_results[CKO_results$pVal <= 0.05,], "CKO_phosphoproteome_all_significant_RAIN2.csv", row.names = F)

write.csv(WT_results[WT_results$pVal > 0.05,], "WT_phosphoproteome_all_insignificant_RAIN2.csv", row.names = F)
write.csv(CKO_results[CKO_results$pVal > 0.05,], "CKO_phosphoproteome_all_insignificant_RAIN2.csv", row.names = F)

write.csv(WT_results, "WT.p_RAIN_results2.csv", row.names = F)
write.csv(CKO_results, "CKO.p_RAIN_results2.csv", row.names = F)
```
